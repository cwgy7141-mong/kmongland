...
export async function createTutorApplication(app: Omit<TutorApplication, 'createdAt'>): Promise<void> {
  const ref = doc(db, 'tutor_applications', app.id);
  await setDoc(ref, {
    ...app,
    createdAt: serverTimestamp(),
  });
